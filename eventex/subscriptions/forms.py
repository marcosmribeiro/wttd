...
class PhoneField(forms.MultiValueField):
    ...
    def __init__(self, *args, **kwargs):
        fields = (forms.IntegerField(label="DDD"),
                  forms.IntegerField(label="Numero"))
...
class SubscriptionForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        # Tambem poderia ser com lambda
        # words = map(lambda w: w.capitalize(), name.split())

        # Oque normalmente seria assim
        # words = name.split()
    ...
    def clean(self):
        ...
        if not self.cleaned_data.get('email') and \
           not self.cleaned_data.get('phone'):
            raise ValidationError(_('Informe o seu e-mail ou telefone'))
